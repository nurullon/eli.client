<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <a-card
      v-for="item in appointmentsStore.previews"
      :key="item.id"
    ...
    >
      <div class="space-y-4 text-[15px] leading-snug text-gray-800">
        ...
        <div class="pt-3 flex justify-between items-center gap-2 flex-wrap">
          <div class="flex gap-2">
            <a-button type="primary" @click="viewDiseases(item)">
              🧬 Болезни
            </a-button>
            <a-button type="dashed" @click="viewPrescription(item)">
              💊 Назначения
            </a-button>
          </div>
...
function viewDiseases(item) {
  console.log(item.id);
  router.push({ name: 'DiseasesPage', params: { appointmentId: item.id } });
}

function viewPrescription(item) {
  router.push({ name: 'PrescriptionPage', params: { appointmentId: item.id } });
}
</script>
